...
- **[2026-07 세션]** 클라우드 배포·자동알림·아침브리핑, 눌림목(매수자리) 백테스트, **종목 발굴(discovery)**, **종목 비교**, 폰에서 목표가/워치리스트 편집(gitstore), 차트 이미지 알림, **기술적분석 대개편(Hero 결론먼저·아코디언)**, **타임프레임(일/주/월)+터치기반 지지저항+평행채널+200주선**, **텔레그램 알림 간결화**, **오늘의 증시 하루정리(market_wrap)**, 모바일 반응형·사이드바 버튼, 사용가이드 HTML(docs/guide.html + Artifact)
...
- [ ] 다크모드, 거래량 보조지표 토글
- [ ] 게임(fable_game_mvp) GitHub Pages 영구 배포 (현재는 임시 cloudflared 터널)
...
- **환경 시세 특이**: 이 샌드박스 yfinance/업비트 시세는 실제와 다른 시뮬레이션값(삼성 30만대 등). GitHub Actions·Streamlit 클라우드도 동일 소스라 값은 일관됨(즉 매수자리 등 계산은 클라우드에서도 맞게 동작). 실제 배포 환경에선 실시세로 동작 예상.
- **외부 스케줄러 이관(2026-07-28)**: `deploy/cloudflare-worker/`. Cloudflare 크론(5분마다) → GitHub API `workflow_dispatch`. `schedule:` 은 무료 러너에서 조용히 버려지지만 `workflow_dispatch` 는 즉시 실행된다는 점을 이용. 크론 트리거는 1개만 쓰고 무엇을 돌릴지는 worker.js 의 SCHEDULE 표로 판단(무료 플랜 한도 회피). 설치는 `wrangler secret put GH_TOKEN` + `wrangler deploy`, 토큰은 Actions read/write 세밀권한 PAT. 안정 확인 후 각 워크플로우의 `schedule:` 제거 예정(그 전까지 병행해도 concurrency + 상태기반이라 중복 발송 없음). 저장소가 PUBLIC 이라 실행 횟수 증가(하루 ~230회)에도 Actions 비용 없음.
- **⏰ 스케줄러 토큰 만료 (2027-08 경)**: Cloudflare 워커가 GitHub 을 깨우는 데 쓰는 fine-grained PAT 는 2026-08-03 발급 · 유효기간 1년. **만료되면 알림이 조용히 멈춘다** (에러 알림 없음 — 워커는 계속 돌지만 dispatch 가 401/400 으로 실패). 증상: 아침 08:30 브리핑이 오지 않음. 확인: `wrangler tail` 에서 dispatch 응답 코드, 또는 `gh run list` 에 workflow_dispatch 기록이 끊겼는지. 해결: 새 PAT 발급(kimdike/jusik + Actions: Read and write) → Cloudflare 대시보드 Workers & Pages → jusik-scheduler → Settings → Variables and Secrets → GH_TOKEN 교체. 진단이 필요하면 DIAG 시크릿을 아무 값으로 추가하면 매 tick 토큰 길이/접두가 로그에 찍힌다.
- **단톡방 분리 운영(2026-07-29~08-03)**: `data/watchlist_group.json` (16종목) — 개인 워치리스트/보유종목과 완전 분리. 개인 포지션 노출 방지 목적이었으나 사용자 확인 결과 단톡방이 부부 2인이라 내용 제약은 없음. `notify.creds(target)` 로 개인/그룹 봇·채팅 분기. 그룹 chat_id 는 개인방으로 폴백하지 않음(조용히 개인방으로 가면 발견이 어렵다). 워크플로우: `group_brief.yml`(08:40 차트), `group_summary.yml`(09:35 텍스트), `spike.yml`(장중 급변동), `market_wrap.yml`(16:00, 양쪽 발송).
- **급변동 감지(check_spikes, 2026-08-03)**: 기준 = 종목별 최근 60일 변동률 90분위, 하한 2%/상한 8%. `watchlist_group.json` 의 `spike_pct` 로 종목별 직접 지정 가능(삼성전자·SK하이닉스 5%). ATR×1.5 로 먼저 만들었다가 실패 — 이 환경 변동성이 커서 삼성전자 기준이 16.3% 로 계산돼 -8.8% 를 놓쳤음. 도배 방지는 그날 최고 '단계' 기록(state 는 Actions 캐시, **커밋 금지**). 6종목 발송 후 60초 대기하고 이어서 발송.
- **텔레그램 속도 제한**: 그룹 채팅 분당 약 20통. 16종목 브리핑(최대 32통)을 연속 발송하면 뒤쪽이 429 로 조용히 누락된다. `notify._post()` 가 `retry_after` 만큼 대기 후 재시도(최대 3회), `build_chart_briefing` 은 종목 간 4초 간격. **발송 함수 반환값을 반드시 로그에 남길 것** — 예전엔 버려서 실패도 "발송 완료"로 찍혔고 워크플로우도 success 였다.
- **개발사 문서**: `docs/history.html` (단독 실행 가능). 06-28~08-03 전체 이력을 '침묵 실패 8건'을 축으로 정리. 처음 보는 사람에게 프로젝트 맥락을 전달하는 용도.
- **반복된 실패 패턴 — 성공 표시를 신뢰하지 말 것**: GitHub schedule 이 success 로 남기고 미실행 / wrangler 가 Success! 로 1글자 토큰 저장 / `alerts_run --test` 가 전송 실패에도 exit 0 / send_photo 실패를 "발송 완료"로 로깅 / spike_state.json 을 커밋해 '급변동 없음' 으로 침묵. **결과를 확인하려면 결과(응답 코드·로그 본문)를 봐야 한다.**
