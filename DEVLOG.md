...
3. **🧪 백테스트** — 신호 전략 / 눌림목(매수자리) 전략 검증. 전략 vs 그냥보유 자산곡선 + 거래횟수/승률/MDD/CAGR.
4. **⚖️ 비교** — 관심·보유 종목 2~4개 신호·수익률·지표·정규화 차트 나란히 비교.
5. **🌡️ 시장 개요** — 주요 지수·환율·VIX 등락률 + 코인 공포탐욕 + 거시 뉴스
6. **💰 내 자산** — 보유종목 수익률·자산비중(원화 환산)
7. **⭐ 워치리스트** — 이름 검색 추가/삭제(폰 영구저장) + 신호 모아보기
8. **🔔 알림 설정** — 신호변화/목표가/매수자리/손절가 텔레그램 알림(현재가+버튼), 테스트·수동점검
9. **📖 사용 가이드** — 사이드바 버튼으로 진입, 사용법·메뉴·용어 설명
