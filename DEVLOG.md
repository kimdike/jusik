...
1. **📊 기술적 분석** (핵심) — 이름검색 → **타임프레임(일/주/월봉)** → 결론3카드 → 차트(이동평균·일목 + **터치기반 지지/저항(반응점 표시)** + **평행채널 옵션+채널위치%** + **장기이평선(200주선/200일선)**) → 지표요약 → 종합총평 → 전문가의견(괴리율) → 지지/저항 → 긍정·부정 신호 → 관련뉴스. (get_ohlcv timeframe=D/W/M: COIN 업비트 days/weeks/months, 주식 yfinance interval)
